-- MVP orchestrator: run_job / run_task (source of truth for execution), idempotency,
-- workflow engine marker, broker pool + leases, referral conversions, ingest alerts.

-- ---------------------------------------------------------------------------
-- run_jobs: one dispatch request (global orchestration unit per experiment batch)
-- ---------------------------------------------------------------------------
...
CREATE UNIQUE INDEX IF NOT EXISTS run_jobs_user_idempotency_idx
  ON public.run_jobs (user_id, idempotency_key)
...
CREATE POLICY "Users can view run_jobs for own experiments"
  ON public.run_jobs FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.experiments e
    WHERE e.id = run_jobs.experiment_id AND e.user_id = auth.uid()
  ));

CREATE POLICY "Anyone can view run_jobs for public experiments"
  ON public.run_jobs FOR SELECT TO anon, authenticated
  USING (EXISTS (
    SELECT 1 FROM public.experiments e
    WHERE e.id = run_jobs.experiment_id AND e.is_public = true
  ));

CREATE TRIGGER update_run_jobs_updated_at
  BEFORE UPDATE ON public.run_jobs
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- ---------------------------------------------------------------------------
-- Broker pool (MVP: single default account row per tool for audit trail)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.broker_pool_accounts (
  ...
  status TEXT NOT NULL DEFAULT 'active'
    CHECK (status IN ('active', 'draining', 'suspended')),
  health_score NUMERIC NOT NULL DEFAULT 1.0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (tool_id, label)
);

INSERT INTO public.broker_pool_accounts (tool_id, label, status, health_score)
VALUES ('v0', 'default', 'active', 1.0)
ON CONFLICT (tool_id, label) DO NOTHING;

-- ---------------------------------------------------------------------------
-- run_tasks: per-builder unit; status machine (plan-aligned labels)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS run_tasks_experiment_tool_idx
  ON public.run_tasks (experiment_id, tool_id, created_at DESC);
...
CREATE POLICY "Users can view run_tasks for own experiments"
  ON public.run_tasks FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.experiments e
    WHERE e.id = run_tasks.experiment_id AND e.user_id = auth.uid()
  ));

CREATE POLICY "Anyone can view run_tasks for public experiments"
  ON public.run_tasks FOR SELECT TO anon, authenticated
  USING (EXISTS (
    SELECT 1 FROM public.experiments e
    WHERE e.id = run_tasks.experiment_id AND e.is_public = true
  ));

CREATE TRIGGER update_run_tasks_updated_at
  BEFORE UPDATE ON public.run_tasks
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- ---------------------------------------------------------------------------
-- broker_account_leases: audit who/when/for which run_task (pool account used)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS broker_account_leases_task_idx
  ON public.broker_account_leases (run_task_id);

ALTER TABLE public.broker_account_leases ENABLE ROW LEVEL SECURITY;

-- No client policies: broker lease data is operational; service role only.

DO $cf$
BEGIN
  ALTER TABLE public.run_tasks
    ADD CONSTRAINT run_tasks_broker_lease_id_fkey
...
-- ---------------------------------------------------------------------------
-- Referral conversions (handoff attribution; clicks stay in referral_clicks)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS referral_conversions_user_created_idx
...
-- ---------------------------------------------------------------------------
-- Builder ingest / RAG alerts (checksum & doc signals; written by sync-builder-data)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS builder_ingest_alerts_tool_created_idx
  ON public.builder_ingest_alerts (tool_id, created_at DESC);

ALTER TABLE public.builder_ingest_alerts ENABLE ROW LEVEL SECURITY;
-- Internal ops: no SELECT for anon/authenticated by default (service role bypasses RLS).

-- ---------------------------------------------------------------------------
-- Realtime: task row updates for Run Center / Compare
-- ---------------------------------------------------------------------------
DO $pub$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'run_tasks'
  ) THEN
