-- Sprint 3 AG: benchmark scores, social (votes/comments), materialized leaderboard.
-- MV uses one row per tool_id; UNIQUE(tool_id) enables REFRESH MATERIALIZED VIEW CONCURRENTLY.

-- ---------------------------------------------------------------------------
-- builder_benchmark_scores: one row per builder_result (PVI + 10 dimensions)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.builder_benchmark_scores (
  ...
  CONSTRAINT builder_benchmark_scores_speed_range
    CHECK (score_speed IS NULL OR (score_speed >= 0 AND score_speed <= 100)),
  CONSTRAINT builder_benchmark_scores_reliability_range
    CHECK (score_reliability IS NULL OR (score_reliability >= 0 AND score_reliability <= 100)),
  CONSTRAINT builder_benchmark_scores_cost_range
    CHECK (score_cost_efficiency IS NULL OR (score_cost_efficiency >= 0 AND score_cost_efficiency <= 100)),
  CONSTRAINT builder_benchmark_scores_deploy_range
    CHECK (score_deploy_readiness IS NULL OR (score_deploy_readiness >= 0 AND score_deploy_readiness <= 100)),
  CONSTRAINT builder_benchmark_scores_mobile_range
    CHECK (score_mobile_responsiveness IS NULL OR (score_mobile_responsiveness >= 0 AND score_mobile_responsiveness <= 100)),
  CONSTRAINT builder_benchmark_scores_a11y_range
    CHECK (score_accessibility IS NULL OR (score_accessibility >= 0 AND score_accessibility <= 100)),
  CONSTRAINT builder_benchmark_scores_vitals_range
    CHECK (score_web_vitals IS NULL OR (score_web_vitals >= 0 AND score_web_vitals <= 100)),
  CONSTRAINT builder_benchmark_scores_uiq_range
    CHECK (score_ui_quality IS NULL OR (score_ui_quality >= 0 AND score_ui_quality <= 100)),
  CONSTRAINT builder_benchmark_scores_complete_range
    CHECK (score_completeness IS NULL OR (score_completeness >= 0 AND score_completeness <= 100)),
  CONSTRAINT builder_benchmark_scores_code_range
    CHECK (score_code_quality IS NULL OR (score_code_quality >= 0 AND score_code_quality <= 100)),
  CONSTRAINT builder_benchmark_scores_pvi_range
    CHECK (pvi_score IS NULL OR (pvi_score >= 0 AND pvi_score <= 100)),

  UNIQUE (builder_result_id)
);

CREATE INDEX IF NOT EXISTS builder_benchmark_scores_tool_id_idx
  ON public.builder_benchmark_scores (tool_id);
CREATE INDEX IF NOT EXISTS builder_benchmark_scores_experiment_id_idx
  ON public.builder_benchmark_scores (experiment_id);
CREATE INDEX IF NOT EXISTS builder_benchmark_scores_scored_at_idx
  ON public.builder_benchmark_scores (scored_at DESC);
...
-- ---------------------------------------------------------------------------
-- user_votes: simple up/down per result (pairwise Arena deferred — see docs)
-- ---------------------------------------------------------------------------
...
CREATE UNIQUE INDEX IF NOT EXISTS user_votes_user_result_kind_uidx
  ON public.user_votes (user_id, builder_result_id, vote_kind);
...
-- ---------------------------------------------------------------------------
-- Materialized leaderboard: one row per tool (UNIQUE(tool_id) for CONCURRENTLY)
-- ---------------------------------------------------------------------------
...
COMMENT ON MATERIALIZED VIEW public.builder_leaderboard IS
  'Aggregated PVI per tool. Refresh: REFRESH MATERIALIZED VIEW CONCURRENTLY public.builder_leaderboard; requires UNIQUE on tool_id.';
